  ...
  !
    ...
    if (present(idx)) then                   !idx supplied?
       ...
       if (counter == 1) then
          ...
          indices = find(q /= cshift(q,1), counter)
       endif
       unique = idx(indices)
       deallocate(q)
       deallocate(indices)
    else
       if (count(array /= cshift(array, 1)) /= counter .and. &
           counter > 1) then
           print *, 'uniq: error dimensions not correct ',counter, &
              count(array /= cshift(array, 1))
           return
        endif
        if (counter == 1) then
           unique = 1
        else
           unique = find(array /= cshift(array, 1), counter)
        endif
     endif
  end function r_uniq
...
!!          indices = 1
!!       else
!!          indices = find(q /= eoshift(q,1), counter)
!!       endif
!!       unique = idx(indices)
!!       deallocate(q)
!!       deallocate(indices)
!!    else
!!       if (count(array /= eoshift(array, 1)) /= counter &
!!          .and. counter > 1) then
!!          print *, 'uniq: error dimensions not correct ',counter, &
!!             count(array /= eoshift(array, 1))
!!          return
!!       endif
!!       if (counter == 1) then
!!          unique = 1
!!       else
!!          unique = find(array /= eoshift(array, 1), counter)
!!       endif
!!    endif
!!  end function i_uniq
  ...
  function r_nuniq( array, idx ) result (counter)
    ...
    if (present(idx)) then                   !idx supplied?
       allocate(q(0:size(array)-1))
       q = array(idx)
       counter = count(q /= cshift(q,1) )
    else
       counter = count(array /= cshift(array,1) )
    endif
...
!!       allocate(q(0:size(array)-1))
!!       q = array(idx)
!!       counter = count(q /= eoshift(q,1) )
!!    else
!!       counter = count(array /= eoshift(array,1) )
!!    endif
!!    counter = max(1,counter)
